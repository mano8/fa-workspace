...
The tracked project settings contain only portable, reviewed project-wide
allow, confirmation, and deny boundaries. Unreviewed personal approvals, paths,
and memory stay in ignored local storage. Auto-memory is disabled for this
project. Add a path-scoped rule only after measuring its need; put a conditional
procedure in a skill rather than an always-loaded rule.
