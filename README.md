...
The full operating contract is in [`.workspace/README.md`](.workspace/README.md). It documents semantic ownership, repository/task scope and authorization, capability evidence, native versus injected delivery, standalone modes, secure runtime/receipts, byte accounting, overrides, budgets, and the distinction between instruction context, reasoning effort, output compression, and price. Current canonical delivery covers the verified Codex devcontainer non-interactive row and the two verified Claude devcontainer non-interactive rows (hook `additionalContext` and full-content `--append-system-prompt`, selected by `model_visible_total`); interactive Claude/Codex rows are `OPTIONAL`/`LIMITED`, a repository scope without its own recorded Claude project trust still fails closed with `E_TRUST`, and unavailable Windows/host-POSIX rows are `UNSUPPORTED`.
